...
/** `holnapFelirat`: pénteken és szombaton „Hétfő” — a /m/holnap a következő munkanapot mutatja. */
export function MTabbar({ tabok, holnapFelirat }: { tabok: MTab[]; holnapFelirat?: string }) {
  const p = usePathname();
  return (
      ...
      {tabok.map((t) => {
        ...
        return (
          <Link key={t.href} href={t.href} className={`flex flex-1 flex-col items-center gap-0.5 py-2.5 text-[11px] font-semibold ${aktiv ? "text-[var(--m-mint)]" : "text-[var(--m-muted)]"}`}>
            <I className="size-5" />
            {t.href === "/m/holnap" && holnapFelirat ? holnapFelirat : t.label}
          </Link>
        );
      })}
    </nav>
